...
Idea central (validada sobre el dataset):
  ...
  - Compuertas que se RELAJAN con el tiempo: temprano solo se decide lo obvio;
    a los ~1200 ms (cuando los humanos ya hablaron y se separan) cae la gran ola
    de buzón. Para colgar se exige PERSISTENCIA (el puntaje venía bajo antes),
    así un humano que baja un instante no se pierde.
...
IMPORTANTE: coeficientes y compuertas calibrados sobre 464 audios (humanos
truncados). Antes de producción RECALIBRA con audio real. silero_vad.onnx debe
estar junto a este archivo (o ajusta la ruta en AMDConfig).
"""
